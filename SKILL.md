...
Plan the main reading direction and give branches, merges and auxiliary paths their own space before polishing the objects. Prefer clean, aligned routes that can be traced from source to destination; if a passage becomes crowded, adjust the layout rather than squeezing more bends into it. See [routing and visual economy](references/routing-and-visual-economy.md) for practical suggestions.

Use short panel titles and nearby object labels. Let visual correspondences explain the mechanism; move longer qualifications, implementation details and repeated explanations into the caption or accompanying notes. Keep labels that are necessary to distinguish the science.
...
Native diagrams and generated illustrations can work together. Graph nodes, masks, embedding strips and operators often benefit from native vector construction. [Asset notes](references/illustrations-and-data-panels.md) and [visual vocabulary](references/native-primitives.md) are optional aids. Consider families of related elements—stacked vectors, several cells or coordinated graph views—using restrained color, fill or opacity differences to show their roles. Keep these variations semantically consistent and legible; repeated marks need not imply exact counts. Use enough meaningful visual material to explain the paper without turning it into a collage.
...
Use an independent subagent review when available: provide the target-method evidence, reference figures and actual rendered outputs. Ask for specific scientific or visual shortcomings, especially ambiguous arrow origins, crowded or crossing routes, text-heavy panels, monotonous glyphs, ornamental complexity and weak differences between versions. Have the reviewer trace the actual visible paths, not merely confirm that the intended relations exist in the source. Repair consequential findings, rerender and have the affected parts rechecked. If subagents are unavailable, perform the comparison yourself and state that limitation.
