...
Use a two-stage workflow: create an editable `.drawio` first draft with native objects or mxGraph XML, then open it in the draw.io app and refine the rendered figure through the available computer-use tools. In the app, adjust connector endpoints and waypoints, alignment, spacing, labels and styles as needed; verify the visible result after changes and save the edited source before exporting. The app-saved file becomes authoritative: do not rerun the initial generator over these edits. Work in a versioned copy when preserving a selected draft. Keep text, connectors, modules and representation glyphs editable. If app control is unavailable or fails, state the limitation; file-based repairs remain a fallback, but never describe them as app edits. The optional [scene helper](references/scene-format.md) supports small native diagrams but is not a limit on the figure or a required pipeline.

Keep arrows easy to follow: prefer straight connections and align connected objects or ports so a tiny offset does not produce a stepped line. Use bends only where a real branch, merge or obstacle makes them useful; avoid unnecessary zigzags, crossings and lines through labels. Graph-internal links can usually be direct segments rather than right-angle routes. Inspect the exported result, since automatic routing may introduce bends that were not intended.
...
Use repetition, layering, color and opacity only when they clarify scientific meaning; none is required for visual richness. Default to a single shape for a single module. In particular, do not stack copies of an encoder box just to make it look richer: that can imply multiple encoders, an ensemble or architectural layers. A stack is appropriate only when it represents an actual collection or repeated structure (such as distinct views, samples or documented layers), and its meaning is apparent from the figure or a concise label. Multiple inputs passing through one shared encoder do not mean multiple encoders. If the meaning of the repeated shapes cannot be explained, remove them. Use color and opacity to distinguish roles, groups or states without inventing quantities or uncertain relationships. Keep figure labels concise; move extended explanations to the caption.
